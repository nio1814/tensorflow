...
#define EXTRACT_AND_VERIFY_DIMENSIONS(label)                                   \
  ...
  const int64 input_planes_raw = GetTensorDim(input_shape, data_format_, 'D'); \
  OP_REQUIRES(                                                                 \
      context,                                                                 \
      FastBoundsCheck(input_planes_raw, std::numeric_limits<int32>::max()),    \
      errors::InvalidArgument("Input planes too large"));                      \
  const int32 input_planes = static_cast<int32>(input_planes_raw);             \
  const int64 filter_rows = filter_shape.dim_size(0);                          \
  const int64 filter_cols = filter_shape.dim_size(1);                          \
  const int64 filter_planes = filter_shape.dim_size(2);                        \
  ...
  const int64 output_planes_raw =                                              \
      GetTensorDim(out_backprop.shape(), data_format_, 'W');                   \
  OP_REQUIRES(                                                                 \
      context,                                                                 \
      FastBoundsCheck(output_planes_raw, std::numeric_limits<int32>::max()),   \
      errors::InvalidArgument("Output planes too large"));                     \
  const int32 output_planes = static_cast<int32>(output_planes_raw);               \
  ...
  int64 pad_rows = 0, pad_cols = 0, pad_planes = 0;                            \
  ...
  OP_REQUIRES_OK(context,                                                      \
                 GetWindowedOutputSize(input_planes, filter_planes, stride,    \
  ...
  OP_REQUIRES(                                                                 \
      context, output_planes == out_planes,                                    \
      errors::InvalidArgument(                                                 \
          label, ": Number of planes of out_backprop doesn't match computed: " \
          "actual = ", output_planes, ", computed = ", out_planes));           \
  Depthwise3DArgs args;                                                        \
  args.batch = batch;                                                          \
  args.in_rows = input_rows;                                                   \
  args.in_cols = input_cols;                                                   \
  args.in_planes = input_planes;                                               \
  args.in_depth = in_depth;                                                    \
  args.filter_rows = filter_rows;                                              \
  args.filter_cols = filter_cols;                                              \
  args.filter_planes = filter_planes;                                          \
  args.depth_multiplier = depth_multiplier;                                    \
  args.stride = stride;                                                        \
  args.pad_rows = pad_rows;                                                    \
  args.pad_cols = pad_cols;                                                    \
  args.pad_planes = pad_planes;                                                \
  args.out_rows = out_rows;                                                    \
  args.out_cols = out_cols;                                                    \
  args.out_planes = out_planes;                                                \
  args.out_depth = out_depth;                                                  \
  VLOG(2) << "DepthwiseConv2d: " << label << " Input: [" << batch << ", "      \
          << input_rows << ", " << input_cols << ", " << input_planes << ", "  \
          << in_depth                                                          \
...
//
// EX:
//   in_depth = 3, depth_multiplier = 2, filter [2, 2, 2], register_width = 4
//
//   'out_backprop': [batch, out_rows, out_cols, out_planes, out_depth]
//
...
//
template <typename T>
static void CopyOutputBackpropRegion(const Depthwise3DArgs& args,
                                     const int64 padded_filter_inner_dim_size,
                                     const int64 in_r, const int64 in_c, const int64 in_p,
  ...
  const int64 out_p_start = std::max(
      static_cast<int64>(0), (in_p - filter_planes + pad_planes + stride) / stride);
  const int64 out_p_end = std::min(out_planes - 1, (in_c + pad_planes) / stride);
  ...
  if ((out_r_end - out_r_start + 1) < args.filter_rows ||
      ...
      (out_p_end - out_p_start + 1) < args.filter_planes) {
    memset(buffer, 0,
  ...
  for (int out_r = out_r_start; out_r <= out_r_end; ++out_r) {
    ...
    for (int out_c = out_c_start; out_c <= out_c_end; ++out_c) {
      ...
      for (int out_p = out_p_start; out_p <= out_p_end; ++out_p) {
...
template <typename T>
static void ComputeBackpropInput(const Depthwise3DArgs& args,
                                 const int64 padded_filter_inner_dim_size,
                                 const int64 in_r, const int64 in_c, const int64 in_p,
...
template <typename T>
struct LaunchDepthwiseConv3dBackpropInputOp<CPUDevice, T> {
  typedef typename Eigen::internal::packet_traits<T>::type Packet;
      ...
      for (int64 b = start; b < limit; ++b) {
        for (int64 in_r = 0; in_r < args.in_rows; ++in_r) {
          for (int64 in_c = 0; in_c < args.in_cols; ++in_c) {
            for (int64 in_p = 0; in_p < args.in_planes; ++in_p) {
              // Populate 'out_bprop_buf' from local 'out_backprop' region.
              CopyOutputBackpropRegion<T>(
                    args, padded_filter_inner_dim_size, in_r, in_c, in_p,
              ...
              ComputeBackpropInput<T>(args, padded_filter_inner_dim_size, in_r,
                                      in_c, in_p, filter_data, out_bprop_buf,
                                      in_bprop_buf,
...
template <typename T>
static void DepthwiseConvBackpropInputReference(const Depthwise3DArgs& args,
...
class DepthwiseConv2dNativeBackpropInputOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    EXTRACT_AND_VERIFY_DIMENSIONS("DepthwiseConv3DBackpropInput");
    ...
    LaunchDepthwiseConv3dBackpropInputOp<Device, T>()(
...
//
template <typename T>
static void ComputeBackpropFilter(const Depthwise3DArgs& args,
...
template <typename T>
static void DepthwiseConvBackpropFilterReference(const Depthwise3DArgs& args,
                                                 const T* out_backprop,
                                                 const T* input,
                                                 T* filter_backprop) {
...
class DepthwiseConv2dNativeBackpropFilterOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    EXTRACT_AND_VERIFY_DIMENSIONS("DepthwiseConv3DBackpropFilter");
