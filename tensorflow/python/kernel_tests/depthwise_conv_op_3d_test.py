...
def depthwise_conv3d(input, filter, strides, padding, rate=None, name=None, data_format=None):
  with ops.op_scope(name, "depthwise", [input, filter]) as name:
    ...
    if in_channels == 1:
      return gen_nn_ops.conv3d(input, filter, strides, padding, name=name, data_format=data_format)
    else:
      ...
      for channel in xrange(in_channels):
        with ops.name_scope("depth%d" % channel) as channel_scope:
          t_in = array_ops.slice(input, [0, 0, 0, 0, channel], [-1, -1, -1, -1, 1],
                                 name="slice_inputs")
          f_in = array_ops.slice(filter, [0, 0, channel, 0], [-1, -1, 1, -1, -1],
                                 name="slice_params")
        ...
        return array_ops.concat(4, convs, name=name)
...
class DepthwiseConv3DTest(test.TestCase):

  # This is testing that depthwise_conv3d and depthwise_conv3d_native
  # produce the same results.  It also tests that NCHW and NWHC
  # formats agree, by comparing the depthwise_conv3d_native with
  ...
  def _VerifyValues(self,
    ...
    with self.test_session(use_gpu=use_gpu) as sess:
      ...
      conv_native = nn_ops.depthwise_conv3d_native(
          native_t1,
          t2,
          strides=strides,
      ...
      conv_interface = nn_impl.depthwise_conv3d(
  ...
  def testDepthwiseConv3D(self):
    for index, (input_size, filter_size, _, stride,
                padding) in enumerate(ConfigsToTest()):
      print("Testing DepthwiseConv3D,", index, "th config:", input_size, "*",
            filter_size, "stride:", stride, "padding:", padding)
      for data_type in [dtypes.float16, dtypes.float32, dtypes.float64]:
        self._VerifyValues(
            input_size, filter_size, stride, padding, data_type, use_gpu=True)

  def testDepthwiseConv3DFormat(self):
    if not test.is_gpu_available():
      return

    for index, (input_size, filter_size, _, stride,
                padding) in enumerate(ConfigsToTest()):
      print("Testing DepthwiseConv3DFormat,", index, "th config:", input_size,
            "*", filter_size, "stride:", stride, "padding:", padding)
      for data_type in [dtypes.float16, dtypes.float32, dtypes.float64]:
        self._VerifyValues(
            input_size,
...
# This is testing against hand calculated results.
  ...
  #     conv = nn_ops.depthwise_conv3d_native(
  #         t1, t2, strides=[1, stride, stride, 1], padding=padding)
  #     value = sess.run(conv)
  #   print("value = ", value)
  ...
  #       strides = [1, 1, stride, stride]
  #
  #     depthwise_conv3d = nn_ops.depthwise_conv3d_native(
  #         native_input,
  #         filter_tensor,
  #         strides,
  #         padding,
  ...
  #
  #     self.assertEqual(output_shape, depthwise_conv3d.get_shape())
  #     if test_input:
  #       err = gradient_checker.compute_gradient_error(
  #           native_input, input_shape, depthwise_conv3d, output_shape)
  #     else:
  ...
  #
  # def testDepthwiseConv3DInputGrad(self):
  #   for index, (input_size, filter_size, output_size, stride,
  #               padding) in enumerate(CheckGradConfigsToTest()):
  #     print("Testing DepthwiseConv3DInputGrad,", index, "th config:",
  #           input_size, "*", filter_size, "stride:", stride, "padding:",
  #           padding)
  #     # Note: float16 test for DepthwiseConv3DInputGrad is not enabled,
  ...
  #           use_gpu=True)
  #
  # def testDepthwiseConv3DInputGradFormat(self):
  #   if not test.is_gpu_available():
  #     return
  #
  #   for index, (input_size, filter_size, output_size, stride,
  #               padding) in enumerate(CheckGradConfigsToTest()):
  #     print("Testing DepthwiseConv3DInputGradFormat,", index, "th config:",
  #           input_size, "*", filter_size, "stride:", stride, "padding:",
  #           padding)
  #     # Note: float16 test for DepthwiseConv3DInputGradFormat is not enabled,
  #     # calculations are not very precise.
  #     for data_type in [dtypes.float32, dtypes.float64]:
  #       self._ConstructAndTestGradient(
  #           input_size,
  ...
  #           data_format="NCHW")
  #
  # def testDepthwiseConv3DFilterGrad(self):
  #   for index, (input_size, filter_size, output_size, stride,
  #               padding) in enumerate(CheckGradConfigsToTest()):
  #     print("Testing DepthwiseConv3DFilterGrad,", index, "th config:",
  #           input_size, "*", filter_size, "stride:", stride, "padding:",
  #           padding)
  #     # Note: float16 test for DepthwiseConv3DFilterGrad is not enabled,
  #     # calculations are not very precise.
  #     for data_type in [dtypes.float32, dtypes.float64]:
  #       self._ConstructAndTestGradient(
  #           input_size,
  ...
  #           use_gpu=True)
  #
  # def testDepthwiseConv3DFilterGradFormat(self):
  #   if not test.is_gpu_available():
  #     return
  #
  #   for index, (input_size, filter_size, output_size, stride,
  #               padding) in enumerate(CheckGradConfigsToTest()):
  #     print("Testing DepthwiseConv3DFilterGradFormat,", index, "th config:",
  #           input_size, "*", filter_size, "stride:", stride, "padding:",
  #           padding)
  #     # Note: float16 test for DepthwiseConv3DFilterGradFormat is not enabled,
  #     # calculations are not very precise.
  #     for data_type in [dtypes.float32, dtypes.float64]:
  #       self._ConstructAndTestGradient(
  #           input_size,
  ...
  #       backprop = nn_ops.depthwise_conv3d_native_backprop_input(
  ...
  #   self.assertAllClose(cpu_value, gpu_value, rtol=1e-4, atol=1e-4)
  #
  # def testDepthwiseConv3DInputGradCompare(self):
  #   for index, (input_size, filter_size, output_size, stride,
  #               padding) in enumerate(ConfigsToTest()):
  #     print("Testing DepthwiseConv3DInputGradCompare,", index, "th config:",
  #           input_size, "*", filter_size, "stride:", stride, "padding:",
  #           padding)
  ...
  #       t2 = constant_op.constant(x2, shape=output_sizes)
  #       backprop = nn_ops.depthwise_conv3d_native_backprop_filter(
  #           t0, t1, t2, strides=[1, stride, stride, 1], padding=padding)
  #       ret = backprop.eval()
  #       self.assertShapeEqual(ret, backprop)
  #       return ret
  ...
  #   self.assertAllClose(cpu_value, gpu_value, rtol=1e-4, atol=1e-4)
  #
  # def testDepthwiseConv3DFilterGradCompare(self):
  #   for index, (input_size, filter_size, output_size, stride,
  #               padding) in enumerate(ConfigsToTest()):
  #     print("Testing DepthwiseConv3DFilterGradCompare,", index, "th config:",
  #           input_size, "*", filter_size, "stride:", stride, "padding:",
  #           padding)
